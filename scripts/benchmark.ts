 ...
 * (cold < 10 s / 1000 files, RSS < 500 MB), plus the size of every tool
 * response the index can serve (see `response-probe.ts`).
 *
 * Usage:
 ...
 *   pnpm benchmark /path/to/flutter/repo --no-responses   # index metrics only
 *
 * History lives in benchmarks/history.jsonl — §9.5 says keep it; a >2×
 * regression between entries is the alarm threshold (§9.3), for response size
 * as much as for index time.
 */
import { appendFileSync, mkdirSync, rmSync } from 'node:fs';
import { join, resolve } from 'node:path';
import { buildIndex } from '../src/index/indexer.js';
import { measureResponses, probeCalls, type ResponseMeasurement } from './response-probe.js';
...
async function main(): Promise<void> {
  ...
  const responses = args.includes('--no-responses')
    ? []
    : await measureResponses(index, probeCalls(index));
  ...
  const result = {
    ...
    responses,
    responseTotalChars: responses.reduce((sum, r) => sum + r.chars, 0),
    widestResponse: widest(responses),
    budgets: {
...
/** The costliest response, the one a token budget has to accommodate. */
function widest(responses: ResponseMeasurement[]): ResponseMeasurement | undefined {
  return responses.reduce<ResponseMeasurement | undefined>(
    (max, r) => (!max || r.chars > max.chars ? r : max),
    undefined,
  );
}
